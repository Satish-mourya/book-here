  ...
  const getAvailableSlots = async ()=>{
    ...
    for(let i=0;i<7;i++){
      ...
      while(currentDate<endTime){
        ...
        // checks that the slot is booked or not
        ...
        <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
            ...
            <div className="flex gap-3 items-center w-full overflow-x-scroll mt-4">
              {
                docSlots.length && docSlots.map((item,idx)=>(
                    ...
                    <div onClick={()=>setSlotIndex(idx)} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex===idx? "bg-blue-500 text-white ":"border border-gray-200" }`} key={idx} >
            ...
            <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
              {docSlots.length && docSlots[slotIndex].map((item,idx)=>(
                <p onClick={()=>setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time===slotTime ? 'bg-blue-500 text-white':'text-gray-400 border border-gray-300' }`} key={idx}>
